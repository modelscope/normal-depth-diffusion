...
class Conv2dSameExport(nn.Conv2d):
    ...
    def forward(self, x):
        input_size = x.size()[-2:]
        if self.pad is None:
            pad_arg = _same_pad_arg(input_size,
                                    self.weight.size()[-2:], self.stride,
                                    self.dilation)
...
def create_conv2d_pad(in_chs, out_chs, kernel_size, **kwargs):
    ...
    if is_dynamic:
        ...
        return nn.Conv2d(
            in_chs, out_chs, kernel_size, padding=padding, **kwargs)
...
def get_condconv_initializer(initializer, num_experts, expert_shape):
    ...
    def condconv_initializer(weight):
        ...
        if (len(weight.shape) != 2 or weight.shape[0] != num_experts
...
class CondConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight_shape = (self.out_channels, self.in_channels
    ...
    def forward(self, x, routing_weights):
        ...
        new_weight_shape = (B * self.out_channels,
