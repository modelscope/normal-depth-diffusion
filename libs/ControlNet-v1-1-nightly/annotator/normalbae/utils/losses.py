...
class compute_loss(nn.Module):
    ...
    def forward_R(self, norm_out, gt_norm, gt_norm_mask):
        ...
        if self.loss_type == 'L1':
            ...
            l2 = torch.sum(
                torch.square(gt_norm - pred_norm), dim=1, keepdim=True)
    ...
    def forward_UG(self, pred_list, coord_list, gt_norm, gt_norm_mask):
        ...
        for (pred, coord) in zip(pred_list, coord_list):
            if coord is None:
                pred = F.interpolate(
                    pred,
                    size=[gt_norm.size(2), gt_norm.size(3)],
                    mode='bilinear',
                    align_corners=True)
                ...
                # coord: B, 1, N, 2
                # pred: B, 4, N
                gt_norm_ = F.grid_sample(
                    gt_norm, coord, mode='nearest',
                    align_corners=True)  # (B, 3, 1, N)
                gt_norm_mask_ = F.grid_sample(
                    gt_norm_mask.float(),
                    coord,
                    mode='nearest',
                ...
                if self.loss_type == 'UG_NLL_vMF':
                    dot = torch.cosine_similarity(
                        pred_norm, gt_norm_, dim=1)  # (B, N)
