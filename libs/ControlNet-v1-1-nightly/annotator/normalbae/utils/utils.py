...
def concat_image(image_path_list, concat_image_path):
    imgs = [
        Image.open(i).convert('RGB').resize((640, 480),
...
def compute_normal_errors(total_normal_errors):
    metrics = {
        'mean':
        np.average(total_normal_errors),
        'median':
        np.median(total_normal_errors),
        'rmse':
        np.sqrt(
            np.sum(total_normal_errors * total_normal_errors)
            / total_normal_errors.shape),
        'a1':
        100.0 *
        (np.sum(total_normal_errors < 5) / total_normal_errors.shape[0]),
        'a2':
        100.0 *
        (np.sum(total_normal_errors < 7.5) / total_normal_errors.shape[0]),
        'a3':
        100.0 *
        (np.sum(total_normal_errors < 11.25) / total_normal_errors.shape[0]),
        'a4':
        100.0 *
        (np.sum(total_normal_errors < 22.5) / total_normal_errors.shape[0]),
        'a5':
        100.0 *
        (np.sum(total_normal_errors < 30) / total_normal_errors.shape[0])
    }
    return metrics
...
def log_normal_errors(metrics, where_to_write, first_line):
    ...
    print('%.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f' %
          (metrics['mean'], metrics['median'], metrics['rmse'], metrics['a1'],
           metrics['a2'], metrics['a3'], metrics['a4'], metrics['a5']))
    ...
    with open(where_to_write, 'a') as f:
        ...
        f.write('%.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n\n' %
                (metrics['mean'], metrics['median'], metrics['rmse'],
                 metrics['a1'], metrics['a2'], metrics['a3'], metrics['a4'],
                 metrics['a5']))
...
def visualize(args, img, gt_norm, gt_norm_mask, norm_out_list, total_iter):
    ...
    for norm_out in norm_out_list:
        norm_out = F.interpolate(
            norm_out, size=[gt_norm.size(2), gt_norm.size(3)], mode='nearest')
        pred_norm = norm_out[:, :3, :, :]  # (B, 3, H, W)
        pred_norm = pred_norm.detach().cpu().permute(0, 2, 3,
                                                     1).numpy()  # (B, H, W, 3)
        pred_norm_list.append(pred_norm)

        pred_kappa = norm_out[:, 3:, :, :]  # (B, 1, H, W)
        pred_kappa = pred_kappa.detach().cpu().permute(
            0, 2, 3, 1).numpy()  # (B, H, W, 1)
        pred_kappa_list.append(pred_kappa)

    # to numpy arrays
    img = img.detach().cpu().permute(0, 2, 3, 1).numpy()  # (B, H, W, 3)
    gt_norm = gt_norm.detach().cpu().permute(0, 2, 3,
                                             1).numpy()  # (B, H, W, 3)
    gt_norm_mask = gt_norm_mask.detach().cpu().permute(
        0, 2, 3, 1).numpy()  # (B, H, W, 1)
    ...
    for i in range(len(pred_norm_list)):
        ...
        target_path = '%s/%08d_pred_norm_%d.jpg' % (args.exp_vis_dir,
                                                    total_iter, i)
        plt.imsave(target_path, pred_norm_rgb)

        pred_kappa = pred_kappa_list[i]
        pred_alpha = kappa_to_alpha(pred_kappa)
        target_path = '%s/%08d_pred_alpha_%d.jpg' % (args.exp_vis_dir,
                                                     total_iter, i)
        plt.imsave(
            target_path, pred_alpha[0, :, :, 0], vmin=0, vmax=60, cmap='jet')
        ...
        E = E * gt_norm_mask
        target_path = '%s/%08d_pred_error_%d.jpg' % (args.exp_vis_dir,
                                                     total_iter, i)
