...
def unflatten_with_named_tensor(input, dim, sizes):
    ...
    new_shape = list(input.shape)[:dim] + list(sizes) + list(
        input.shape)[dim + 1:]
...
def forward_vit(pretrained, x):
    ...
    unflattened_size = (
        int(
            torch.div(
                h, pretrained.model.patch_size[1], rounding_mode='floor')),
        int(
            torch.div(
                w, pretrained.model.patch_size[0], rounding_mode='floor')),
    )
    ...
    if layer_3.ndim == 3:
        layer_3 = unflatten_with_named_tensor(layer_3, unflattened_dim,
                                              unflattened_size)
    if layer_4.ndim == 3:
        layer_4 = unflatten_with_named_tensor(layer_4, unflattened_dim,
                                              unflattened_size)
