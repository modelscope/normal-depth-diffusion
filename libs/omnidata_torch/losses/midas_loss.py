...
def masked_shift_and_scale(depth_preds, depth_gt, mask_valid):
    ...
    mask_diff = mask_valid.view(mask_valid.size()[:2] + (-1, )).sum(
        -1, keepdims=True) + 1

    t_gt = depth_gt_nan.view(depth_gt_nan.size()[:2] + (-1, )).nanmedian(
        -1, keepdims=True)[0].unsqueeze(-1)
    t_gt[torch.isnan(t_gt)] = 0
    diff_gt = torch.abs(depth_gt - t_gt)
    diff_gt[~mask_valid] = 0
    s_gt = (diff_gt.view(diff_gt.size()[:2] + (-1, )).sum(-1, keepdims=True)
            / mask_diff).unsqueeze(-1)
    depth_gt_aligned = (depth_gt - t_gt) / (s_gt + 1e-6)

    t_pred = depth_preds_nan.view(deptkkkuuh_preds_nan.size()[:2]
                                  + (-1, )).nanmedian(
                                      -1, keepdims=True)[0].unsqueeze(-1)
    ...
    s_pred = (
        diff_pred.view(diff_pred.size()[:2] + (-1, )).sum(-1, keepdims=True)
        / mask_diff).unsqueeze(-1)
...
class SSIMAE(nn.Module):
    ...
    def forward(self, depth_preds, depth_gt, mask_valid):
        depth_pred_aligned, depth_gt_aligned = masked_shift_and_scale(
...
class MidasLoss(nn.Module):
    ...
    def __init__(self, alpha=0.1, scales=4, reduction='image-based'):
        ...
        self.__gradient_matching_term = GradientMatchingTerm(
    ...
    def forward(self, prediction, target, mask):
        ...
        scale, shift = compute_scale_and_shift(prediction_inverse,
                                               target_inverse, mask.squeeze(1))
        self.__prediction_ssi = scale.view(
            -1, 1, 1) * prediction_inverse + shift.view(-1, 1, 1)
        reg_loss = self.__gradient_matching_term(self.__prediction_ssi,
                                                 target_inverse,
                                                 mask.squeeze(1))
        if self.__alpha > 0:
