...
def process(device, model, model_type, image, input_size, target_size,
            optimize, use_camera):
    """
...
def create_side_by_side(image, depth, grayscale):
    ...
    if not grayscale:
        right_side = cv2.applyColorMap(
            np.uint8(right_side), cv2.COLORMAP_INFERNO)
...
def run(input_path,
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            print('  Processing {} ({}/{})'.format(image_name, index + 1,
                                                   num_images))
            ...
            # compute
            with torch.no_grad():
                prediction = process(device, model, model_type, image,
                                     (net_w, net_h),
            ...
            if output_path is not None:
                ...
                if not side:
                    ...
                    content = create_side_by_side(original_image_bgr * 255,
                                                  prediction, grayscale)
                    cv2.imwrite(filename + '.png', content)
                utils.write_pfm(filename + '.pfm',
            ...
            while True:
                ...
                if frame is not None:
                    original_image_rgb = np.flip(
                        frame, 2)  # in [0, 255] (flip required to get RGB)
                    image = transform({'image':
                                       original_image_rgb / 255})['image']
                    ...
                    original_image_bgr = np.flip(original_image_rgb,
                                                 2) if side else None
                    content = create_side_by_side(original_image_bgr,
                                                  prediction, grayscale)
                    cv2.imshow(
                    ...
                    if output_path is not None:
                        filename = os.path.join(
                            output_path, 'Camera' + '-' + model_type + '_'
                            + str(frame_index))
                        cv2.imwrite(filename + '.png', content)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        'Folder with input images (if no input path is specified, images are tried to be grabbed '
        'from camera)')
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        'Preferred height of images feed into the encoder during inference. Note that the '
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'Option to resize images to a square resolution by changing their widths when images are '
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'Use a grayscale colormap instead of the inferno one. Although the inferno colormap, '
    ...
    run(args.input_path, args.output_path, args.model_weights, args.model_type,
        args.optimize, args.side, args.height, args.square, args.grayscale)
