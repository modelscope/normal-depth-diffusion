...
def _make_levit_backbone(model, hooks=[3, 11, 21], patch_grid=[14, 14]):
    ...
    pretrained.act_postprocess2 = nn.Sequential(
        ...
        nn.Unflatten(
            2, torch.Size(
                (np.ceil(patch_grid_size / 2).astype(int)).tolist())))
    pretrained.act_postprocess3 = nn.Sequential(
        Transpose(1, 2),
        nn.Unflatten(
            2, torch.Size(
                (np.ceil(patch_grid_size / 4).astype(int)).tolist())))
...
class ConvTransposeNorm(nn.Sequential):
    ...
    def fuse(self):
        ...
        b = bn.bias - bn.running_mean * bn.weight / (bn.running_var
                                                     + bn.eps)**0.5
...
def stem_b4_transpose(in_chs, out_chs, activation):
    ...
    return nn.Sequential(
        ConvTransposeNorm(in_chs, out_chs, 3, 2, 1), activation(),
        ConvTransposeNorm(out_chs, out_chs // 2, 3, 2, 1), activation())
