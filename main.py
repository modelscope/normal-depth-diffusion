...
import torch
import torchvision
from ldm.callbacks.checkpoint_cb import GlobalCallback
...
class SetupCallback(Callback):
    ...
    def on_exception(self, trainer, pl_module, exception):
        ...
        def isKeyboardError(exception):
            return type(exception) == KeyboardInterrupt

        if trainer.global_rank == 0 and isKeyboardError(exception):
...
def safety_resume(path):
    # NOTE that the model we have prior model that need to be pop
    resume_weight = torch.load(path, map_location='cpu')

    # pop ignore_weights
    sd = resume_weight['state_dict']
    keys = list(sd.keys())
    has_ignore = False
    for k in keys:
        for ik in ['prior_model']:
            if k.startswith(ik):
                print('Deleting key {} from state_dict.'.format(k))
                del sd[k]
                has_ignore = True
    resume_weight['state_dict'] = sd

    if has_ignore:
        torch.save(resume_weight, path)
...
if __name__ == '__main__':
    ...
    if opt.resume:
        ...
        _tmp = logdir.split('/')
        nowname = _tmp[-1]
        # pop prior model-ckpt, as we set the prior model as the Torch-model.
        safety_resume(opt.resume)
    ...
    default_callbacks_cfg = {
        ...
        'global_callback': {
            'target': 'ldm.callbacks.checkpoint_cb.GlobalCallback',
            'params': {
                'resume':
                opt.resume,
                'now':
    ...
    # plugin logger apex add speeds

    try:
        # if MixedPrecisionPlugin would lead Nan Loss
        # trainer_kwargs["plugins"] = MixedPrecisionPlugin('16', device = 'cuda',scaler=torch.cuda.amp.GradScaler())
        pass
    except:
        assert version.parse(pl.__version__) <= version.parse('1.4.2')
    ...
    if not cpu:
        ...
        pl.seed_everything(opt.seed + get_rank())
        print(get_rank())
    else:
        ngpu = 1
