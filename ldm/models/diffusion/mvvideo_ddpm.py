...
this is vae ddpm
-- merci
Video DDPM denoise in latent space following MVDream
produced by Lingteng Qiu
"""
import pdb
...
class anneal_warmup():
    ...
    def __init__(self, anneal_ratio, anneal_global_step, num_steps):
        self.anneal_ratio = anneal_ratio
        self.anneal_global_step = anneal_global_step
        self.steps = num_steps // (len(anneal_global_step) + 1)
        self.start_steps = self.steps

    def __call__(self, x, global_step):

        if (torch.rand(1) > self.anneal_ratio).item():
            return x
        else:
            return int(self.start_steps + self.start_steps
                       * upper_bound(self.anneal_global_step, global_step))
...
class DDPM(pl.LightningModule):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        print('=================', self.cond_method)
        # if self.cond_method.startswith("cat"):
        if 'cat' in self.cond_method:
            old_state = sd
            new_state = self.state_dict()

            # Check if we need to port weights from 4ch input to 8ch
            in_filters_load = old_state[
            ...
            if in_shape != in_filters_load.shape:
                ...
                for input_key in input_keys:
                    ...
                    if input_weight.size() != old_state[input_key].size():
                        ...
                        input_weight.zero_()
                        input_weight[:, :4, :, :].copy_(
                            old_state[input_key][:, :4])
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.cond_method = cond_method
        assert self.cond_method in [
            'ori', 'cat_n', 'cat_d', 'cat_nd', 'img_crossattn',
            'img_crossattn_cat', 'img_cat'
        ]
        self.uncond_ratio = uncond_ratio
        print(
            f'==================cond_method:{self.cond_method}, uncond:{self.uncond_ratio}=================='
        )
        ...
        # TODO cannot optimize if freeze crossattn?
    ...
    def read_data_from_key(self, bs, batch, k):
    ...
    def get_input(self,
        ...
        if self.cond_method == 'ori':
            pass
        elif self.cond_method == 'img_crossattn':
            batch_size, chs, img_h, img_w = x.shape
            real_batch_size = batch_size // self.num_frames
            x_split = x.reshape(real_batch_size, self.num_frames, chs, img_h,
                                img_w)
            img_cond = x_split[:, 0]

        elif self.cond_method == 'img_crossattn_cat':
            batch_size, chs, img_h, img_w = x.shape
            real_batch_size = batch_size // self.num_frames
            x_split = x.reshape(real_batch_size, self.num_frames, chs, img_h,
                                img_w)
            ...
            img_z = self.encode_first_stage(img_cond).mode().detach()
            # cond['c_concat'] = img_z.repeat_interleave(self.num_frames, dim=0)
            concat_img_z = torch.zeros_like(
                img_z.repeat_interleave(self.num_frames, dim=0))
            for b_idx in range(real_batch_size):
                concat_img_z[b_idx * self.num_frames] = img_z[b_idx]
            cond['c_concat'] = concat_img_z

        elif self.cond_method == 'img_cat':
            batch_size, chs, img_h, img_w = x.shape
            real_batch_size = batch_size // self.num_frames
            x_split = x.reshape(real_batch_size, self.num_frames, chs, img_h,
                                img_w)
        ...
        elif self.cond_method == 'cat_n':
            normal = self.read_data_from_key(bs, batch, 'normal')

            normal_z = torch.nn.functional.interpolate(
                normal, size=(_h // 8, _w // 8), mode='nearest')

            cond['c_concat'] = normal_z

        elif self.cond_method == 'cat_d':
            depth = self.read_data_from_key(bs, batch, 'depth')

            depth_z = torch.nn.functional.interpolate(
                depth, size=(_h // 8, _w // 8), mode='nearest')

            cond['c_concat'] = depth_z

        elif self.cond_method == 'cat_nd':
            normal = self.read_data_from_key(bs, batch, 'normal')
            depth = self.read_data_from_key(bs, batch, 'depth')
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c = c.repeat_interleave(self.num_frames, dim=0)
                    # adding num_frames times
                else:
        ...
        if not eval_phase:  #training phase
            # To support classifier-free guidance, randomly drop out only text conditioning 10% like sd-v1.5
            real_batch_size = x.size(0) // self.num_frames
            random = torch.rand(real_batch_size, device=x.device)
            prompt_mask = rearrange(random < uncond, 'n -> n 1 1')
            prompt_mask = prompt_mask.repeat_interleave(self.num_frames, dim=0)
        ...
        if self.cond_method.startswith('img_crossattn'):
            img_context = self.cond_stage_model.encode_image(img_cond)
            img_context = img_context.repeat_interleave(self.num_frames, dim=0)
            cond['img_context'] = img_context
            cc = torch.cat([cc, img_context], dim=1)

        cond['context'] = cc
        cond['num_frames'] = self.num_frames
        cond['camera'] = camera  # B 16
    ...
    def shared_step(self, batch, **kwargs):
        ...
        for key in batch.keys():
            if isinstance(batch[key], torch.Tensor):
                if len(batch[key].shape) == 5:  # image normal depth albedo ic
                    batch[key] = rearrange(batch[key],
                                           'b n h w c -> (b n) h w c')
                elif len(batch[key].shape) == 3:  # camera
                    batch[key] = rearrange(batch[key], 'b n  c -> (b n) c')

        x, c = self.get_input(
            batch, self.first_stage_key, uncond=self.uncond_ratio)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, 'split_input_params'):
            raise NotImplementedError('Video ddpm dose not implement it !')
        else:
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            if sampler_type == 'dpmsolver':
                ...
                ddim_sampler = DDIMSampler(self)  # TODO support plms
            else:
                raise NotImplementedError
            if 'unconditional_guidance_scale' in kwargs:
                ...
                if scale != 1.0:
                    if not cond['context'].shape[1] == 1:  # prompt condition
                        uc = self.get_learned_conditioning(batch_size * [''])
                    else:  # image condition
                        uc = torch.zeros_like(cond)
                    kwargs['unconditional_conditioning'] = {}
                    kwargs['unconditional_conditioning']['context'] = uc
                    kwargs['unconditional_conditioning']['camera'] = cond[
                        'camera']
                    kwargs['unconditional_conditioning']['num_frames'] = cond[
                        'num_frames']
                    if 'c_concat' in cond:
                        kwargs['unconditional_conditioning'][
                            'c_concat'] = cond['c_concat']
                    if 'img_context' in cond:
                        kwargs['unconditional_conditioning'][
                            'context'] = torch.cat([uc, cond['img_context']],
                                                   dim=1)
                        kwargs['unconditional_conditioning'][
                            'img_context'] = cond['img_context']
    ...
    def log_rgbd(self,
        ...
        # need to flattent caption to visualized
        captions = batch['caption']
        mv_captions = []
        for caption in captions:
            mv_captions.extend([caption] * self.num_frames)

        batch['caption'] = mv_captions
    ...
    def log_images(self,
        ...
        def decode_show(img):
            # return repeat(img[:, 0, ...], 'b h w -> b c h w', c = 3)
            return img
        ...
        n_row = min(x.shape[0], n_row)
        log['inputs'] = x
        log['inputs_n'] = self.read_data_from_key(N, batch, 'normal')
        log['inputs_d'] = self.read_data_from_key(N, batch, 'depth')
        log['reconstruction'] = decode_show(xrec)
    ...
    def configure_optimizers(self):
        ...
        ######################hard code#############
        if hasattr(self.cond_stage_model, 'img_feat_proj'):
            params = params + list(
                self.cond_stage_model.img_feat_proj.parameters())
        ###########################################
        if self.learn_logvar:
    ...
    def forward(self,
        ...
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(
                ...
                context=context,
                camera=camera,
                num_frames=torch.tensor(num_frames).to(camera))
