...
class LatentDiffusion(DDPM):
    ...
    def get_input(self,
        ...
        _, _c, _h, _w = x.shape
        z = F.interpolate(
            x, (_w // 8, _h // 8), mode='bilinear', align_corners=False)
        ...
        if return_first_stage_outputs:
            xrec = F.interpolate(
                z, (_w, _h), mode='bilinear', align_corners=False)
            out.extend([x, xrec])
        if return_original_cond:
            out.append(xc)
    ...
    def log_rgbd(self,
        ...
        if sample:
            ...
            x_samples = F.interpolate(
                samples, (_w * 8, _h * 8), mode='bilinear')
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, context=None, camera=None, num_frames=1):
        ''' context: [B, 77, N]
