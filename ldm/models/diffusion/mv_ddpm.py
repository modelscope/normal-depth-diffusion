...
class DDPM(pl.LightningModule):
    ...
    @torch.no_grad()
    def prior_inference(self, inputs, prior_inputs, is_objaverse):
        # depth prior model
        # midas or zoe is 384 model

        inputs = inputs[is_objaverse == False]
        prior_inputs = prior_inputs[is_objaverse == False]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.prior_model is not None:
            ...
            if (batch['objaverse'] == False).sum() == 0:
                img = torch.cat([batch['image'], batch['disparity']], dim=-1)
            else:
                laion_inference = self.prior_inference(batch['image'],
                                                       batch['prior'],
                                                       batch['objaverse'])
                img = torch.cat([batch['image'], batch['disparity']], dim=-1)

                img[batch['objaverse'] == False] = laion_inference
            batch['image'] = img
    ...
    def log_rgbd(self,
        ...
        if batch['image'].shape[-1] == 3:
            ...
            if self.prior_model is not None:
                # [-1,1]
                if (batch['objaverse'] == False).sum() == 0:
                    img = torch.cat([batch['image'], batch['disparity']],
                                    dim=-1)
                else:
                    laion_inference = self.prior_inference(
                        batch['image'], batch['prior'], batch['objaverse'])
                    img = torch.cat([batch['image'], batch['disparity']],
                                    dim=-1)
                    img[batch['objaverse'] == False] = laion_inference
                batch['image'] = img
...
class Layout2ImgDiffusion(LatentDiffusion):
    ...
    def log_images(self, batch, N=8, *args, **kwargs):
        ...
        for tknzd_bbox in batch[self.cond_stage_key][:N]:
            bboximg = mapper.plot(tknzd_bbox.detach().cpu(), map_fn,
                                  (256, 256))
