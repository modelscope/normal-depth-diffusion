...
class DiffusionWrapper(nn.Module):
    ...
    def __init__(self, diffusion_model, conditioning_key=None):
        super().__init__()
    ...
    def forward(self, x, t, **kwargs):
        if self.conditioning_key is None:
            ...
            context, num_frames, camera = kwargs['context'], kwargs[
                'num_frames'], kwargs['camera']
            out = self.diffusion_model(
                x, t, context=context, num_frames=num_frames, camera=camera)
        elif self.conditioning_key == 'hybrid':
            c_concat, context, num_frames, camera = kwargs['c_concat'], kwargs[
                'context'], kwargs['num_frames'], kwargs['camera']
            xc = torch.cat([x] + [c_concat], dim=1)
            out = self.diffusion_model(
                xc, t, context=context, num_frames=num_frames, camera=camera)
