...
from ldm.util import (filter_nan_loss, filter_nan_loss_pixel,
...
class LPIPSWithDepthDiscriminator(LPIPSWithDiscriminator):
    ...
    def forward(self,
        ...
        if self.perceptual_weight > 0:
            p_loss = 0.
            for input_, recon_ in zip(
                    torch.split(inputs, 3, dim=1),
                    torch.split(reconstructions, 3, dim=1)):
                p_loss += self.perceptual_loss(input_.contiguous(),
                                               recon_.contiguous())
            rec_loss = rec_loss + self.perceptual_weight * p_loss
...
class LPIPSWithDepthDiscriminator(LPIPSWithDiscriminator):
    ...
    def forward(self,
                ...
                split='train',
                weights=None):
        rec_loss = filter_nan_loss_pixel(
