...
from ldm.data.laion_rgb_depth import COCORGBSRTest
...
class cocorgbdataset(COCORGBSRTest):
    ...
    def __getitem__(self, i):
        ...
        return {
            ...
            'ref_img': self.transforms(ref_image),
            'caption': caption,
        }
...
def compute_statistics(dataset, model, batch_size, num_workers, dims, device):
    ...
    pred_ref_arr = np.empty((len(dataset), dims))
    start_idx = 0

    for batch in tqdm(dataloader):

        batch_img = batch['img'].to(device)
        batch_ref = batch['ref_img'].to(device)

        with torch.no_grad():
            pred = model(batch_img)[0]
            pred_ref = model(batch_ref)[0]
        ...
        if pred_ref.size(2) != 1 or pred_ref.size(3) != 1:
            pred_ref = adaptive_avg_pool2d(pred_ref, output_size=(1, 1))

        pred_ref = pred_ref.squeeze(3).squeeze(2).cpu().numpy()
        pred_ref_arr[start_idx:start_idx + pred.shape[0]] = pred_ref
        start_idx = start_idx + pred.shape[0]

    m1 = np.mean(pred_arr, axis=0)
    s1 = np.cov(pred_arr, rowvar=False)

    m2 = np.mean(pred_ref_arr, axis=0)
    s2 = np.cov(pred_ref_arr, rowvar=False)
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    dataset = cocorgbdataset(
        ...
        min_crop_f=.8,
        ref_path=paths,
        transforms=TF.Compose([TF.Resize((299, 299)),
                               TF.ToTensor()]))

    return compute_statistics(dataset, model, batch_size, num_workers, dims,
                              device)
...
def main():
    ...
    if args.save_stats:
        save_fid_stats(args.path, args.batch_size, device, args.dims,
                       num_workers)
        return

    fid_value = calculate_fid_given_paths(args.path, args.batch_size, device,
                                          args.dims, num_workers)
    print('FID: ', fid_value)
