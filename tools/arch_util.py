...
def load_pretrained_network(net, model_path, strict=True, weight_keys=None):
    ...
    print(
        f'Loading pretrained model {net.__class__.__name__} from {model_path}')
...
def symm_pad(im: torch.Tensor, padding: Tuple[int, int, int, int]):
    ...
    def reflect(x, minx, maxx):
        ...
        out = np.where(normed_mod >= rng, double_rng - normed_mod,
                       normed_mod) + minx
...
def excact_padding_2d(x, kernel, stride=1, dilation=1, mode='same'):
    assert len(
        x.shape) == 4, f'Only support 4D tensor input, but got {x.shape}'
    ...
    pad_l, pad_r, pad_t, pad_b = (pad_col // 2, pad_col - pad_col // 2,
...
class ExactPadding2d(nn.Module):
    ...
    def forward(self, x):
        return excact_padding_2d(x, self.kernel, self.stride, self.dilation,
                                 self.mode)
