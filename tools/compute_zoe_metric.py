...
from libs.omnidata_torch.lib.zoe import ZoeDetector
...
def save_outputs(img_path, save_depth, cnt):
    with torch.no_grad():
        save_img = os.path.join('./outputs/zoe_views', img_path.split('/')[-1])
...
for data in tqdm.tqdm(img_files[args.start:args.end]):
    ...
    img_path = data
    save_path = data.replace('rgb_COCO', 'zoe_COCO').replace('.jpg', '.npz')
