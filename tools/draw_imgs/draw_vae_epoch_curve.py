...
if __name__ == '__main__':
    ...
    resume_epoch = opt.resume_epoch.split('/')[-1].split('_')[-1].split('.')[0]
    resume_epoch = int(resume_epoch)

    lightning_config.callbacks.image_logger.params.sub_name = '_{:08d}'.format(
        resume_epoch)
    ...
    # test loading ckpt
    if not opt.train:
        resume = '/'.join(opt.resume.split('/')[:-1])
        # resume_path = os.path.join(resume, "epoch={:06d}.ckpt".format(opt.resume_epoch))
        resume_path = os.path.join(resume, opt.resume_epoch)
        # resume_path = os.path.join(resume, "step_{:08d}.ckpt".format(opt.resume_epoch))
        pl_sd = torch.load(resume_path, map_location='cpu')
